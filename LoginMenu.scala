...
object LoginMenu {
    ...
    def findUser(): Boolean = {
      ...
      //of there combination
      val userinfo = askInfo()
      //if the loginMap is empty it automatically send them back to the menu since there is no user created
      if (loginMap.isEmpty) {
        ...
        try{
          ...
          if(userinfo("password").equals(loginMap(userinfo("username")))){
            ...
            println("user not created or credentials not correct!")
            return false
          }

        }catch{
          case e: IllegalArgumentException => {println("error"); return false;}
          case e: NoSuchElementException => {println("user not created or credentials not correct!");return false;}
        }
